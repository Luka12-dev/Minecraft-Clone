			...
			for z in range(-2, 3):
				...
				for lx in range(chunk.CHUNK_WIDTH):
					for lz in range(chunk.CHUNK_LENGTH):
						world_x = x * chunk.CHUNK_WIDTH + lx
						world_z = z * chunk.CHUNK_LENGTH + lz
						
						# Simple terrain generation with sine waves for hills
						height = int(64 + 
							8 * math.sin(world_x * 0.1) * math.cos(world_z * 0.1) +
							4 * math.sin(world_x * 0.05) * math.cos(world_z * 0.15) +
							3 * math.sin(world_x * 0.2) * math.cos(world_z * 0.2))
						
						# Clamp height
						height = max(50, min(80, height))
						
						# Place blocks
						for ly in range(height):
							if ly == 0:
								# Bedrock at bottom
								self.chunks[chunk_position].blocks[lx][ly][lz] = 7
							elif ly < height - 4:
								# Stone
								self.chunks[chunk_position].blocks[lx][ly][lz] = 1
							elif ly < height - 1:
								# Dirt
								self.chunks[chunk_position].blocks[lx][ly][lz] = 3
							else:
								# Grass on top
								self.chunks[chunk_position].blocks[lx][ly][lz] = 2
						
						# Add some trees randomly
						if random.random() < 0.01 and height < 75:
							tree_height = random.randint(4, 6)
							# Trunk
							for ty in range(tree_height):
								if height + ty < chunk.CHUNK_HEIGHT:
									self.chunks[chunk_position].blocks[lx][height + ty][lz] = 17
							# Leaves
							for leaf_y in range(tree_height - 2, tree_height + 2):
								for leaf_x in range(-2, 3):
									for leaf_z in range(-2, 3):
										if abs(leaf_x) + abs(leaf_z) < 4:
											wx = lx + leaf_x
											wz = lz + leaf_z
											wy = height + leaf_y
											if (wx >= 0 and wx < chunk.CHUNK_WIDTH and 
												wz >= 0 and wz < chunk.CHUNK_LENGTH and 
												wy < chunk.CHUNK_HEIGHT):
												if self.chunks[chunk_position].blocks[wx][wy][wz] == 0:
													self.chunks[chunk_position].blocks[wx][wy][wz] = 18
